...
<div id="section-denovo" class="topic-section">
  ...
  <div class='paper-box'><div class='paper-box-image'>
    ...
    <dd>Jun Xia&dagger;, Sizhe Liu&dagger;, <strong><u>Jingbo Zhou&dagger;</u></strong>, Shaorong Chen, hongxin xiang, Zicheng Liu, Yue Liu, Stan Z. Li
      <p style="color: red;">🔥 NeurIPS 2024 AIDrugX Spotlight</p>
    </dd>
  </div></div>
...
<!-- GNN -->
<div id="section-gnn" class="topic-section hidden">
  <!-- Deep GNN PSNR 模块 -->
  <div class='paper-box'><div class='paper-box-image'>
...
<!-- AIVC（AI for life science / virtual cell / multi-omics等场景） -->
<div id="section-aivc" class="topic-section hidden">
  <!-- GRAPE -->
  <div class='paper-box'><div class='paper-box-image'>
...
function showTopic(topicId) {
  ...
  const btn = document.getElementById('filter-' + topicId);
  if (btn) btn.classList.add('active');

  // 2) 区块显示/隐藏
  document.querySelectorAll('.topic-section').forEach(sec => sec.classList.add('hidden'));
  const target = document.getElementById('section-' + topicId);
  if (target) target.classList.remove('hidden');
}
// 默认显示 de novo
document.addEventListener('DOMContentLoaded', () => showTopic('denovo'));
</script>

<!-- 基础样式（可与站点样式合并） -->
<style>
.filter-link { color:#0066cc; text-decoration:none; padding:4px 8px; font-weight:600; cursor:pointer; }
.filter-link.active { background:#0066cc; color:#fff; border-radius:4px; }
.filter-separator { margin:0 6px; color:#777; }
